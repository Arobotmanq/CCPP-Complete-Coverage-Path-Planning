...
# lib
from mywadl.lib.fence import Fence
from mywadl.lib.route import RouteSet
...
class Maze(Fence):
    ...
    def plotRoutes(self, ax):
        ...
        for route in self.routeSet:
            ...
            wasted_length=route.plot(ax, color=next(cols))
            Sum_length += route.length
            
            wasted_length_sum += wasted_length
            if (route.home in Timedict):
                Timedict[route.home]=Timedict[route.home]+route.ToF
            else:
                Timedict[route.home]=route.ToF   
        print('Sum_length=',Sum_length)
        print('wasted_length_sum=',2*wasted_length_sum)
        DisEfficiency=(Sum_length-2*wasted_length_sum)/Sum_length
        print('DisEfficiency=',DisEfficiency)
        print(Timedict)
        lengthtime = max(Timedict, key=lambda k: Timedict[k]) 
        print(Timedict[lengthtime])
        mytxt=open('PathplanningOut.txt',mode='w')
        mytxt.write("lengthsum %.2f m\n"%Sum_length)
        mytxt.write("lengthtime %.2f s\n"%Timedict[lengthtime])
        mytxt.close()
        mytxt=open('IndexCalculation.txt',mode='a')
        mytxt.write("%s,"%(Sum_length/1000))
        mytxt.write("%s,"%(Timedict[lengthtime]/3600))
        mytxt.write("%s\n"%DisEfficiency)
        mytxt.close()
