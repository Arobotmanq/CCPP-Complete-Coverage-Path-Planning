...
import numpy as np
# lib
from mywadl.lib.parameters import Parameters
...
class RouteSet(object):
    ...
    def check(self, cords):
        ...
        if route.check():
            RouteSet.usedHomes.add(route.home)
            return route
        else:
            return None
    ...
    def write(self, pathDir):
        ...
        """
        # mytxt=open('PathplanningOut.txt',mode='w')
        ...
        for i, route in enumerate(self.routes):
            ...
            #mytxt.write("route%s: %.2f m %.2f  sec\n"%(i,route.length,route.ToF))
            # lengthsum += route.length
            #lengthtime +=route.ToF
        # mytxt.write("lengthsum %.2f m\n"%lengthsum)
        #mytxt.write("lengthtime %.2f s\n"%lengthtime)
        # mytxt.close()
...
class Route(object):
    ...
    def __init__(self, cords, zone, home):
        ...
        self.UTMZone = zone
        RouteSet.usedHomes = RouteSet.usedHomes if RouteSet.usedHomes is not None else set()  # ✅
    ...
    def setHome(self, home):
        ...
        if home is None:
            print("⚠️ Warning: No home provided to setHome()")
            return  # 🔹 如果 home 是 None，直接返回
        homeDist = np.inf
        selected_idx = None
        self.home = None
        print(RouteSet.usedHomes)
        # print("当前已使用的 homes:", Route.usedHomes)

        for h in home:
        #     h_tuple = tuple(h)
        #     # print(self.usedHomes)
        #     if h_tuple in Route.usedHomes: 
        #         # print("已使用的 home，跳过:", h) # 如果 h 已经使用过，则跳过
        #         continue
            if tuple(h) in RouteSet.usedHomes:  # ✅ 如果 home 已经被使用，跳过
                continue
        ...
        #     self.logger.error("❌ 没有可用的 home，所有 home 都已被使用！")
        #     return  # 直接返回，避免 `None` 进入 `usedHomes`

        
        # selected_home = self.home
        # Route.usedHomes.add(tuple(selected_home))  # 标记 home 为已使用
        # # print("更新后已使用的 homes:", Route.usedHomes)
        # self.logger.debug(f"Home set to {self.home}")
        # # shift and wrap
        if self.home is None:
            print("❌ 没有可用的 home，所有 home 都已被使用！")
            return
        
        self.GPScords = self.GPScords[selected_idx:] + self.GPScords[1:selected_idx+1]
        # sync the utm
        self.GPS2UTM()
    ...
    def plot(self, ax, color='b'):
        ...
        if self.home is not None:
            ...
            mylat, mylng = self.GPScords[0]
            myHlat, myHlng  = self.home
            wasted_length=self.DistGPS([mylat, mylng],[myHlat, myHlng],70,70)
            # print(mydist,self.home)
        return wasted_length
